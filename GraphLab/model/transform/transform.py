...
def ego_nets(graph, radius=2):
    ...
    # Store all node features in a dictionary
    node_features = {node: feature for node, feature in enumerate(graph.node_feature)}
    node_label = {node: node_label for node, node_label in enumerate(graph.node_label)}
    patch_id = {node: patch_id for node, patch_id in enumerate(graph.patch_id)}
    ...
    for i in range(len(egos)):
        ...
        graph.patch_id = torch.cat((graph.patch_id, patch_id_map), dim=0)
