...
from GraphLab.contrib.transform.identity import compute_identity
...
class FeatureAugment(nn.Module):
    ...
    def _bin_features(self, graph, key, bin_edges, feature_dim=2, as_label=False):
        ...
        graph[key] = self._one_hot_tensor(feat,
                                          one_hot_dim=feature_dim,
                                          as_label=as_label)

    def _one_hot_tensor(self, vals, one_hot_dim=1, as_label=False):
    ...
    def _orig_features(self, graph, key):
    ...
    def _position_features(self, graph,
                           key,
                           feature_dim=4,
                           scale=1,
                           wavelength=10000):
    ...
    def _get_max_value(self, dataset, feature_key):
        list_scalars = np.concatenate([g[feature_key] for g in dataset])
        return np.max(list_scalars)

    def _get_bin_edges(self, dataset, feature_key, feature_dim, bin_method):
        """
    ...
    def _augment_feature(self,
        ...
        for key, dim in zip(features, feature_dims):
            ...
            if key not in dataset[0]:
                ...
                scale = dim / 2 / self._get_max_value(
                    dataset, key)
                dataset.apply_transform(self._position_features,
                                        update_graph=True,
                                        update_tensor=False,
                                        key=key,
                                        feature_dim=dim,
